...
class Problems
{
  ...
  /// <summary>
  /// Tabulation:
  /// Example for 3 integers { A, B, C }:
  /// - First loop: find max for subproblem { A, B }: finds max of A, A + B, or B
  /// - Second loop find max for subproblem { previous max, C } 
  /// </summary>
  ...
  */
  ...
  // Auxiliary Space Complexity: O(1)
  /// <summary>
  /// - We want to keep track of the start and end index of where there is
  /// the max numbers of consecutive ones.
  /// - For any range of indices, there can only be max of N number of zeros to flip.
  /// - If there are more zeros than we are allowed to flip, then we need to adjust the 
  /// starting index (while loop)
  /// </summary>
  /// <param name="arr"></param>
  /// <param name="N"></param>
  /// <returns></returns>
  public static int BitFlip(int[] arr, int N)
  {
    ...
    var startIndex = 0;
    var zeros = 0;
    for (var endIndex = 0; endIndex < arr.Length; endIndex++)
    {
      if (arr[endIndex] == 0)
      {
        zeros++;
