...
class Graph
{
  ...
  {
    if (!isVertex(id1) || !isVertex(id2)) return false;
    storage[id1].Add(id2);
    return true;
  }

  // Time Complexity: O(E) bc loop through edges to find id to remove.
  // Auxiliary Space Complexity: O(1)
  public bool removeEdge(int id1, int id2)
  {
    if (!isVertex(id1)) return false;
    storage[id1] = storage[id1].Where(s => s != id2).ToList();
    return true;
  }
  ...
  {
    storage = storage ?? new Dictionary<int, List<int>>();
